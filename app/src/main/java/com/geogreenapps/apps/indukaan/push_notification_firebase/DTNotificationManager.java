...
import com.geogreenapps.apps.indukaan.dtmessenger.InComingDataParserSender;
import com.geogreenapps.apps.indukaan.utils.NotificationUtils;
...
public class DTNotificationManager {
    ...
    public void parse(String incomingData) {
        ...
        if (mParser.getType().equals(Parser.PRODUCT)) {
            ...
            boolean notif_products = PreferenceManager.getDefaultSharedPreferences(context)
                    .getBoolean("notif_products", true);

            if (notif_products)
                NotificationUtils.sendNotification(campaign_id, context, mParser.getTitle(), mParser.getBody().getStoreName(), icon, bigImageUrl, ProductDetailActivity.class, data);
        ...
        }else if (mParser.getType().equals(Parser.OFFER)) {
